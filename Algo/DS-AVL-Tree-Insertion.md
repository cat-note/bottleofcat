...
2. 插入完成后，借助**弹栈**来沿着插入时比较的各节点**回到整棵树的根节点**(回溯):
...
💡 实际上，在回溯过程中，如果发现某节点的`平衡因子 = 0`，就可以**不用再回溯**了。

究其原因，咱们得关注一下插入前和插入后的**平衡因子**变化情况: 

1. 插入前，AVL树中**每颗子树都是平衡的**，也就是说，所有节点的平衡因子都在`[-1, 1]`范围内。

2. 树中某**节点A**的平衡因子 $BF \in \left [ -1,1 \right ]$ ，就意味着节点A的**左子树和右子树的高度差**不会超过1。
