...
// [归并排序]
// 归并排序是贯彻了【分治】思想的排序算法，而分治算法往往会采用递归实现(便于理解)，当然，归并排序也是可以用非递归方式写的
// 归并排序递归体中做的事主要是下面三件:
//      1. 将序列【二分】为两个序列 (分)
//      2. 分别对两个序列进行排序 (治)
//      3. 然后将排序好的两个序列合并成【一个有序的序列】 (并)
// 其中比较特殊的是过程3，需要借助一个额外的数组buf来合并两个序列成一个有序序列(详见代码)
// 因此空间复杂度是O(n)，归并排序是【非原地算法】！
// 归并排序的时间复杂度很稳定，就是O(nlogn)，因此排序速度是较快的，但很明显，代价就是要更多的空间支持
// 归并排序其实是稳定的算法，它适用于数据量【较大】，同时对【排序结果稳定性】有要求的场景
