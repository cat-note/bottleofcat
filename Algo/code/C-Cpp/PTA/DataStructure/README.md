...
|7-43|[字符串关键字的散列映射](https://pintia.cn/problem-sets/15/exam/problems/890)| [DS-7-43.cpp](./DS-7-43.cpp)<br>如果PTA采用的编译器有BUG，可以用这个题解:<br>[DS-7-43-alternative.cpp](./DS-7-43-alternative.cpp) | 字符串哈希表的实现。字符串哈希常用的方法就是将字符串看作**某种进制的数**，并将其转换为**十进制数**再进行哈希。<br>本题采用的碰撞处理方法是**平方探测法**，值得注意的是**向前探测**的时候**小心下标越界**。 | √ |
