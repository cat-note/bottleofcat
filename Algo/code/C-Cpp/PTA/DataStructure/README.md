...
|7-22|[堆栈模拟队列](https://pintia.cn/problem-sets/15/exam/problems/837)| [DS-7-22.cpp](./DS-7-22.cpp) | 利用**容量不同**的**两个栈**实现一个队列。两个栈分为**入队栈**和**出队栈**，队列的所有元素并不是都要储存在入队栈中，入队栈满时**借助出队栈**也可以储存一些元素。因此，**较小的那个栈**决定了**队列容量的上限**。 |  |
