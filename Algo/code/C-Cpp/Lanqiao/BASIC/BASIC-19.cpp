            ...
            为什么要这样做？要是我先将这个落单字符交换到了正中间，可能在处理其他字符时，【处于正中间的落单字符】会被【再次移动】，处理起来会麻烦很多！
                来，举个【反例】，对于字符串abccadd:
                    [1] abccadd - former指向首个字符a, symmetry指向最后一个字符d
                        ↑     ↑
                    [2] abccdda - 这里略过latter找到字符a并移动的过程 (交换2次)
                        ↑     ↑
                    ...
                    [4] accbdda - 把b移动到正中间, former+1,symmetry-1，可以发现从这里开始就不对劲了，有个c字符被忽略了！
                          ↑ ↑
                    [4] accbdda - 【尝试回溯一下former】，让former指回第一个c字符，symmetry也回到对称的位置上
                         ↑   ↑
                    [5] acbddca - 这里略过latter找到第二个c字符并交换到symmetry位置的过程。可以发现交换过程中，【落单字符b被影响了！】
                         ↑   ↑
                    [6] acbddca - former+1,symmetry-1。此时former再次指向落单字符b，但【程序认为之前已经存在落单字符】了，因此这里会判断为无法形成回文串
                          ↑ ↑
                    [7] 返回Impossible - 显然这是错误的结果！
                实际上，我们可以【不移动b】，从上面第[3]步重新开始：
                    [3] abccdda - 不移动b，只计算b移动到中间需要的交换次数(abccdda->accbdda, 交换2次)
                         ↑   ↑
                    [4] abccdda - 💡注意这关键一步，接下来只做了former+1, 而【没有改变symmetry】
                          ↑  ↑     (你可以想象成现在字符串是accbdda，former指向第一个c，与之对称的是倒数第二个字符d)
                    [5] abcddca - 仍然是通过latter寻找到第二个c，并交换到symmetry处 (交换2次)
                          ↑  ↑
                    [6] abcddca - former+1, symmetry-1，可以发现d和d已经对称了，不做交换操作
                           ↑↑
                    [7] 最后将b移动到中间(abcddca->acdbdca), 这一步其实程序中没有写，因为【题目只需要知道总共要交换几次】。
                        (b的交换次数在第[3]步已经算了)
                        最终算出：
                            a交换的次数+c交换的次数+b交换的次数=2+2+2=6次
        围绕这三点，算法怎么写就显而易见了~
