...
while (true) {
    ...
    if (!running) {
        ...
        } else if (globalTimer - runningStart >= timeSlice) { // 时间片轮转
...
for (let i in illustrations) {
    let [arriveTime, turn] = finishTime[i];
    sum += turn;
    processNum++;
    console.log(`${i} ${illustrations[i].join('')}    %c${turn}ms (Arrive at ${arriveTime}ms)`, 'color:#585858');
}
