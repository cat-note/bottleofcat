...
while (true) {
    ...
    if (!running) {
        ...
        readyQueue.sort((a, b) => {
            let aSpend = a.spend,
                bSpend = b.spend, // 要求服务时间
                aarrivalTime = a.arrivalTime,
                barrivalTime = b.arrivalTime, // 到达时间
                aWaitTime = globalTimer - aarrivalTime,
                bWaitTime = globalTimer - barrivalTime, // 等待时间
                aRatio = (aWaitTime + aSpend) / aSpend, // 响应比
                bRatio = (bWaitTime + bSpend) / bSpend;
            return bRatio - aRatio; // 从响应比高到响应比低排序
        });
