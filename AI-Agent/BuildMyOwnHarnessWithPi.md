...
自从今年年初开始尝试 AI 辅助编码后，咱是真的一发不可收拾了，这体验简直和一直以来的 Tab 补全完全不同。在本地 Agent 程序的约束和辅助下，指令遵循较强的大语言模型 (LLMs, Large Language Models) 变得可以基于“观察-决策-执行”循环来在一个项目的范围下执行编码任务，而不像以前只能生成、补全代码片段。
...
3. 对于比较危险的修改操作，最好还要**手动或者指示模型对数据进行备份**，不然其灵机一动覆盖或移除了真的很难找回来（建议善用 Git 或者其他版本管理工具）。
4. 尽量结构化，不要挤在一坨，对于较繁杂的需求提示词可以写成 Markdown 文档再让 Agent 读取。

除此之外还可以让模型主动“暴露”它不清晰的地方，一个神器就是 Matt Pocock 的 **grill-me** skill，主打**尽量消除不确定性**，让模型来主动~~拷打~~询问它觉着不清楚的要点。纵使需求给得再详细，总会有些我们认为理应如此的信息是没有提供的，grill-me 很大程度上弥补了这一点缺陷。待需求对齐后再开始执行任务，能让最终成果能更加接近我们的验收标准。  
...
遇到这样宽泛的输入时，按上面“消除输入不确定性”来的话，会深挖 “‘低延迟’到底指什么？”、“助手是否应该拥有长期记忆？”、“Live2D 要做到哪一级同步？” 等这些实现细节。但对于这类大型任务，这样消除不确定性也是不一定够的：**任务粒度很大**，其中大量约束、技术取舍和实现问题并不是一开始就能穷举出来的，而只会在任务执行过程中逐步暴露。如果在一个任务在达到可验证阶段之前，需要连续做出很多相互依赖的决策，那么早期的一处偏差仍可能在任务执行过程中被不断放大，最终可能有一大坨需要我们返工。

因此我们面对大粒度任务输入要做的是先细分成多个小粒度的问题输入，对于每个较小粒度的输入再确定决策点、消除不确定性。Matt Pocock 的 `wayfinder` skill 其实就在做这件事，其会将一个宽泛的输入划分为多个更细粒度的决策 ticket (工单)，可以结合 grilling 这种 skill 来深究每个 ticket 的细节，完成 ticket 后就能得到某一个小点中具体的决策清单。比如上述输入可能会被拆分为:  
...
在完成所有决策 ticket, 得到细粒度的具体决策后才会开始写需求和规范文档（对应 `to-spec` skill），随后则可以根据规范文档垂直切分（对应 `to-tickets` skill）为具体的端到端开发任务（可独立开发、验证和交付的完整小功能），最后分而治之，执行每个开发任务并完成实现和验收。例如上面的示例 Ticket 1 完成决策并形成规范后:  
...
总的来说面对大而模糊的问题转换为小而明确的问题并作出决策，接着根据决策编写规范，划分为细粒度的开发任务，确保开发过程中模型能收到更多反馈，以避免错误积累过多，最终以更低的成本（返工更少）来完成这类复杂的任务。慢即是快。  
...
然而 skills 多了的话，依旧会占用一些上下文空间，有没有**在引入前完全不占用上下文**的呢？可以借鉴 Pi 作者的 “文档 + Bash” 思想，我们能直接把任务指示和调用工具的方法写成简短文档，需要用的时候让模型去读取即可。这里“让模型去读取”的实现也是比较多样的：很多 Agent 客户端支持通过 `@FILE` 语法把文件载入上下文；也有像 Pi 这样支持提示词模板，可以直接把文档转换为斜杠命令的；最朴素的方法当然是直接通过对话让模型去读取某个文档了。  

无论是 skill 还是提示词模板，本质上都是文档 (Documents)，都在告诉模型如何调用工具 (利用 Bash 等 shell 的命令)、怎样在状态间进行流转，归根到底还是应了那句话: *Bash and Documents are All You Need*!  
...
总的来说 MCP 其实更像是模型 Agent 能力还没那么强时的产物（需要细致的规范来约束行为），在如今最新一代模型 Agent 能力更加稳定的情况下，日常编码绝大多数场景其实 Bash + 文档就足够了，且符合渐进式披露的思想，能尽量保持上下文简洁，开销通常更低。就 token 使用效率 (Token Efficiency，可以理解成每个 token 对 Agent 任务执行成功概率的边际贡献，即新加入上下文的 token 是否能尽量提升任务成功的概率而不是降低) 来说显然是 **文档 / 提示词模板 > Skills >> MCP** 的。

----

除此之外还有个省钱小妙招，比如任务执行已经积累了一定的上下文，但因为一些原因要没法继续了（比如 5 小时限制快要达到了，或者眼皮子打仗要睡觉了），这种时候可以使用 `/compact` 来压缩上下文。这样下次恢复 (resume) 上下文执行任务的时候**不会因为前缀缓存大面积失效而掏空钱包**。  

但是要注意的是 `/compact` 必然导致部分信息的丢失，对任务执行可能有一定影响。最新一代模型都对长程 Agent 任务特别进行了优化，通常会自动分解成多个小点来做，这种情况下执行 `/compact` 的最佳时机应该是**某一个小点任务刚刚执行完**。如果在小任务没执行完前就压缩上下文，一方面为了继续执行任务 Agent 又需要去调用工具重新获取很多必要的信息，会产生重复的开销；另一方面则是任务执行的中间状态可能会丢失，导致任务结果与预期产生偏差。最好的实践当然还是今日事今日毕，至少把当前任务跑完再休息吧。顺便一提，拆分任务粒度也是减少单次任务执行时间和降低上下文占用的一种方式。  

> Matt Pocock 的 [`/handoff`](https://github.com/mattpocock/skills/blob/0ab1b63a410a03d3627979a109c8695de27af954/skills/productivity/handoff/SKILL.md) skill 其实也有类似 `/compact` 的功效。不同的是 handoff 不是原地压缩会话上下文，也不会原样保留末几条消息，而是会把会话总结成一个文档来方便递交给其他 Agent 来继续工作。  

### 1.2. Sub-agents 的使用

Sub-agents（子 Agent）是典型的上下文管理手段。

模型上下文工程 (Context Engineering) 中很重要的一点就是尽量减少不必要的干扰，让**模型上下文大部分信息和当前执行的任务相关**，保持简洁。比如在代码仓库诊断 bug 时通常要先扫描仓库代码来理清项目结构和模块依赖关系，这个过程如果在主上下文中进行，会有巨量冗余的内容进入上下文，且在后续修复任务中可能只会修改某几行代码，其余的根本没用上，产生了极大的浪费；更糟的是，过分繁杂的上下文可能会干扰模型对指令的遵循效果，亦或者是很容易触发上下文压缩导致信息丢失，最终致使任务执行结果变差。  
...
2. **执行任务中**：可以并发执行相互**独立**的编码任务。比如主 Agent 写主要代码，子 Agent 来补充单元测试（但如果用测试驱动 (TDD) 开发其实没必要这样做）。
3. **执行任务后**：可以作为第三方视角来执行**独立**的代码审查任务，进行交叉检查。

尤其值得一提的是“执行任务中”的情况，我觉着还是应该尽量把编码这种涉及大量修改的任务放到主 Agent 执行：一方面是可以直接观测到 Agent 进行了哪些操作；另一方面，子 Agent 执行任务的效果**取决于主 Agent 是怎么构造子 Agent 的上下文的**，也就是主 Agent 决定了子 Agent 的局部视野（能看到什么信息），可能经过这一层转述，和原本的指令间发生了偏移，产出的内容不一定是符合验收标准的。且就算要用，也尽量用于编写完全不相关（边界明确）、独立于其他 Agent 任务的模块，否则这种局部视野极有可能因模型对模块边界划分不清，导致多个 Agent 的修改互相冲突，最终堆出一坨💩山。  

因此，Sub-agents 还是更加适合**在主 Agent 执行任务前、后**使用，适合子 Agent 的这些任务都有类似的特性:  

1. 我们**更关注其结论**，中间过程大量信息是冗余、主 Agent 用不到（和主任务无关）的；
2. 不依赖于主任务（耦合度低），独立性强且边界清晰（作为子任务有明确的输入和输出）。
...
Agent 在执行任务时，作为循环中的一步，**清晰的反馈**是非常重要的，其实际上就是在给模型的搜索空间剪枝，即很大程度上决定了 Agent 下一步的行动。反馈一方面是用户的反馈（中途改变方案、补充要求等）；另一方面则是最常见的，工具调用所产生的反馈。

上文中曾多次提到“反馈”一词（可以用 Ctrl + F 搜索看看），大多指的都是工具调用的反馈。个人认为清晰的反馈至少需要以下几点特性：  

1. 语义化的执行状态（成功、失败、阻塞、超时等）；
2. 明确的执行结果（**尤其是失败的情况**，最好能提供详细的错误信息供模型决策，说清楚“为什么”。比如 `execution failed` 提供的反馈显然不如 `execution failed: AuthenticationFailed: access token expired. Retry after refreshing credentials.`）。
3. （如果失败了）可能的恢复方式，比如:  
    ```bash
    $ git ad
...
众所周知，大模型作为概率模型输出难免是有“幻觉”现象的，而交叉检查就是为了尽量减少这种现象对于任务执行结果的影响，上文提到的编码后派出崭新上下文的 Agent 进行审查就是一种交叉检查方式。除了编码外，在进行文档格式、数据分析等任务时最好也使用独立的 Agent 来进行交叉检查，可以使用和执行任务相同的模型或者其他多个水平相近的模型来实现。

**有明确评判指标、验收标准**的任务最适合交叉检查，比如论文的实验数据迁移（从一个模板迁移到另一个模板）、保持功能一致的代码重构、拼写检查等；而**涉及主观评价，比较具有创造性**的任务则不太适合让 Agent 来做交叉检查，如小说、论文写作，语气检查等。单就论文来讲，就算同一个模型去多次审查同一篇论文“是好是坏”，也可能会有不同的评价结果，不同的模型则更可能带有不同的“偏见”。  

交叉检查涉及的内容较多时也可以**切分为更细的粒度**再检查，比如检查一整篇论文的数学符号问题，就可以逐节来进行。执行交叉检查任务的模型会不会有遗漏或者幻觉呢？难说。因此对于比较重要的数据，咱的建议是多检查几次。
...
人都无法脱离不确定性，大模型更难以脱离，因此 AI Agent 虽然在大多时候是非常强大的工具，但若我们驾驭的方式不当，不仅产出的效果不好，也可能造成不菲的损失。咱认为纵使模型自身 Agent 能力再强，Harness 这一套机制也是不会被淘汰掉的。

这篇小记反反复复磨了好一段时间，算是把咱个人的体会（能表达出来的、比较重要的部分）都写上来了，真心希望能帮助到大家吧（；´д｀）ゞ  
