...
<a id="inMem-outPut" for-anchor="true" title="示例输出结果"></a> 
...
* 程序中请不要让函数返回指向**栈内存**的**局部变量的地址**。  

注意：**静态局部变量**除外，因为静态局部变量是**储存在静态储存区的**。  
...
👆 如果之前例子中的`test`函数内这个局部数组变量声明为**静态变量**，程序就可以正常执行了。
