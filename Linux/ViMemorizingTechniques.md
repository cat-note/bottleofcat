...
8. **剪切**  
    ...
    有了复制，就不得不提到**复制并移除**——剪切了。通过这些指令移除的文本会被存进缓冲区，可用于**粘贴**。
...
9. **粘贴**
    ...
    * 👇举例：先执行`ygj`复制**开头没有换行符**的一段文本，然后分别用 `p` 和 `P` 进行粘贴
    ...
    🌟 `命令模式/正常模式`下指令可加数字前缀重复执行，这里也是一样的。比如 `4p` 就是把缓冲区的文本**粘贴四次**。
